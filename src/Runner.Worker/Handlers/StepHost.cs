    ...
    public sealed class ContainerStepHost : RunnerService, IContainerStepHost
    {
                                            ...
                                            CancellationToken cancellationToken)
        {
            ...
            if (string.Equals(Environment.GetEnvironmentVariable(Constants.Variables.Actions.NoSharedVolume), "true", StringComparison.OrdinalIgnoreCase))
            {
                ...
                return await ExecuteViaWorkflowAgentAsync(context, workingDirectory, fileName, arguments, environment, standardInInput, cancellationToken);
            }
        ...
        private async Task<int> ExecuteViaWorkflowAgentAsync(
